...
//	createNodeSectionVBoxForSceneDisplay();
	...
	while (iterator.hasNext()) // Looping though (lines) file
	{
	    ...
	    else if (field[0].equalsIgnoreCase("TextFile"))			{ textFile = field[1]; }
		...
		if ((field[1].equalsIgnoreCase("Object")) && (load))
		{
			...
			else if (function.equalsIgnoreCase("Cylinder"))
			{
				...
				else			    {scoopedNode3dArrayList.add(0, new MyCylinder(this, name+ns, eval(height), new Celestial(celestialName,eval(celestialMass),eval(celestialRadius),celestialGravitational, celestialCollisional, celestialGroup, celestialPolarity), divisions, cullFace, drawMode, color, eval(opacity), cached, ssVerbosity));}
			    }
			    else
			    {
				if (celestialRef.length()>0)   {scoopedNode3dArrayList.add(0, new MyCylinder(this, name+ns, eval(height), Celestials.get(celestialRef,ssVerbosity), divisions, cullFace, drawMode, mapType, getImage(imageFile), cached, ssVerbosity));}
				else			    {scoopedNode3dArrayList.add(0, new MyCylinder(this, name+ns, eval(height), new Celestial(celestialName,eval(celestialMass),eval(celestialRadius),celestialGravitational, celestialCollisional, celestialGroup, celestialPolarity), divisions, cullFace, drawMode, mapType, getImage(imageFile), cached, ssVerbosity));}
			    }
			}
			else if (function.equalsIgnoreCase("Sphere")) // Single Sphere
			{
				...
				else				{scoopedNode3dArrayList.add(0, new MySphere(this, name+ns, new Celestial(celestialName,eval(celestialMass),eval(celestialRadius),celestialGravitational, celestialCollisional, celestialGroup, celestialPolarity), divisions, cullFace, drawMode, color, eval(opacity), cached, ssVerbosity));}
			    }
			    else
			    {
				if (celestialRef.length()>0)	{scoopedNode3dArrayList.add(0, new MySphere(this, name+ns, Celestials.get(celestialRef,ssVerbosity), divisions, cullFace, drawMode, mapType, getImage(imageFile), cached, ssVerbosity));}
				else				{scoopedNode3dArrayList.add(0, new MySphere(this, name+ns, new Celestial(celestialName,eval(celestialMass),eval(celestialRadius),celestialGravitational, celestialCollisional, celestialGroup, celestialPolarity), divisions, cullFace, drawMode, mapType, getImage(imageFile), cached, ssVerbosity));}
			    }
			}
			else if (function.equalsIgnoreCase("HyperSphere")) // Spheres in a HyperSphere shape
			{
			    ...
			    else { scoopedNode3dArrayList.addAll(myHyperSphere.createHyperSphere(new Point3D(0,0,0), shells, eval(shellRadius), eval(shellResolution), eval(shellMotion), eval(randomise), eval(celestialRadius), divisions, cullFace, drawMode, color, eval(opacity)));	}
			}
			else if (function.equalsIgnoreCase("Torus")) // Spheres in Torus shape
			{
			    ...
			    else { scoopedNode3dArrayList.addAll(myHyperSphere.createTorus(new Point3D(0,0,0), eval(torusRotate), shells, eval(shellRadius), eval(shellResolution), eval(shellMotion), eval(randomise), eval(celestialRadius), divisions, cullFace, drawMode, color, eval(opacity), true)); }
			}
...
//		    verbosity = 0;
	...
	else
	{
	    ...
	    Path textfile = defaultfs.getPath(System.getProperty("user.dir"),file);
	...
	Scanner scanner = null; try { scanner = new Scanner(new File(textfile.toString())); } catch (FileNotFoundException ex) { System.out.println("Err file: " + file + ": " + ex); }
